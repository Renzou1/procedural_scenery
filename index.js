...
async function main() {
  ...
  const response = await fetch('hex_grass_bottom.obj');
  ...
  const response2 = await fetch('building_mine_blue.obj');
  const text2 = await response2.text();
  const data2 = parseOBJ(text2);
  const bufferInfo2 = twgl.createBufferInfoFromArrays(gl, data2);
